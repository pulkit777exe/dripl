...
export function useAccessibility({
  ...
  return {
    ...
    announceSelection: (count: number) => {
      ...
      void (announceSelection?.(message) || announce(message));
    },
    announceToolChange: (tool: string) => {
      const message = `${tool} tool activated`;
      void (announceToolChange?.(message) || announce(message));
    },
    announceUndo: () => {
      const message = 'Undone';
      void (announceUndo?.(message) || announce(message));
    },
    announceRedo: () => {
      const message = 'Redone';
      void (announceRedo?.(message) || announce(message));
    },
    announceElementCreated: (type: string) => {
      const message = `${type} created`;
      void (announceElementCreated?.(message) || announce(message));
    },
    announceElementDeleted: (count: number) => {
      const message = count === 1 ? 'Element deleted' : `${count} elements deleted`;
      void (announceElementDeleted?.(message) || announce(message));
    },
  };
}
