...
export function useExport() {
  const toJSON = useCallback((elements: DriplElement[], pretty: boolean = true): string => {
    const exportData = {
      version: 1,
      type: 'dripl-canvas',
  ...
  const toSVG = useCallback((elements: DriplElement[], width: number, height: number): string => {
...
<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}">
  ...
  const toClipboard = useCallback(async (elements: DriplElement[]): Promise<void> => {
  ...
  const downloadFile = useCallback((content: Blob | string, filename: string): void => {
    const blob =
