...
export interface CanvasElement {
  ...
  type:
    ...
    | "freedraw"
    | "text"
    | "ellipse"; // Excalidraw uses "ellipse" instead of "circle"
  x: number;
  y: number;
  width: number;
  height: number;
  ...
  fillStyle?: "solid" | "hachure" | "cross-hatch"; // Excalidraw fill styles

  // Visual properties
  opacity: number;
  roughness: number; // 0 = architect, 1 = artist, 2 = cartoonist
  roundness: number | { type: number }; // Can be number or object
  rotation?: number; // In radians (Excalidraw calls this "angle")
  ...
  // Path properties (for lines, arrows, freedraw)
  points?: Point[];

  // Excalidraw-specific properties
  seed?: number; // Random seed for roughness
  version?: number; // Version number for updates
  versionNonce?: number; // Nonce for version tracking
  isDeleted?: boolean; // Soft delete flag
  groupIds?: string[]; // IDs of groups this element belongs to
  frameId?: string | null; // ID of frame containing this element
  boundElements?: Array<{ id: string; type: string }>; // Elements bound to this one
