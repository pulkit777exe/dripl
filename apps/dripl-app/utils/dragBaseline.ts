...
export function getDragTargetIds(
  ...
  selectedIds.forEach(id => {
    ...
    if (el.type === 'text' && 'containerId' in el && el.containerId) ids.add(el.containerId);
  });
  return ids;
}
...
export function applyDeltaToBaseline(baseline: PointerDownState, delta: Point): DriplElement[] {
...
export function mergeDragPreview(
  ...
  return sceneElements.map(el => (draggedIds.has(el.id) ? (previewById.get(el.id) ?? el) : el));
}
