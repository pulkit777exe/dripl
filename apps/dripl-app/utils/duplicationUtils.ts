 ...
 */
export function duplicateElement(element: DriplElement, offset = DUPLICATION_OFFSET): DriplElement {
...
export function duplicateElements(
  ...
  const nonSelectedElements = elements.filter((el) => !selectedIds.includes(el.id));
...
export function duplicateElementWithBoundText(
  ...
  const boundText = elements.find((el) => el.type === "text" && el.boundElementId === element.id) as TextElement | undefined;
