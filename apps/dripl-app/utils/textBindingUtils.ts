...
import { getBounds, getCenter } from "@dripl/math";
...
export function updateBoundTextPosition(
  ...
  let newX = bounds.x + bounds.width / 2;
  let newY = bounds.y + bounds.height + 5;
...
export function updateAllBoundTextPositions(elements: DriplElement[]): DriplElement[] {
  ...
  const updatedTextElements = textElements.map((textEl) => {
    ...
    if (boundElement) {
      return updateBoundTextPosition(boundElement, textEl);
    }
    return textEl;
  });
