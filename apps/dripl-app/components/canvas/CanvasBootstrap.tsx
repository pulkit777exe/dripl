...
function applyAppStateToStore(appState: Partial<LocalCanvasState> | null) {
  ...
  if (typeof appState.panX === "number" && typeof appState.panY === "number") store.setPan(appState.panX, appState.panY);
  if (appState.currentStrokeColor) store.setCurrentStrokeColor(appState.currentStrokeColor);
  if (appState.currentBackgroundColor) store.setCurrentBackgroundColor(appState.currentBackgroundColor);
  if (typeof appState.currentStrokeWidth === "number") store.setCurrentStrokeWidth(appState.currentStrokeWidth);
  if (typeof appState.currentRoughness === "number") store.setCurrentRoughness(appState.currentRoughness);
  if (appState.currentStrokeStyle && VALID_STROKE_STYLES.has(appState.currentStrokeStyle)) store.setCurrentStrokeStyle(appState.currentStrokeStyle);
  if (appState.currentFillStyle && VALID_FILL_STYLES.has(appState.currentFillStyle)) store.setCurrentFillStyle(appState.currentFillStyle);
  if (appState.activeTool && isActiveTool(appState.activeTool)) store.setActiveTool(appState.activeTool);
}
...
export function CanvasBootstrap(props: CanvasBootstrapProps) {
  ...
  useEffect(() => {
    ...
    setRuntimeStoreSync((next) => {
      useCanvasStore.getState().setElements([...next.elements], { skipHistory: true });
  ...
  useEffect(() => {
    ...
    const bootstrap = async () => {
      ...
      if (mode === "file") {
        const scene = await loadInitialScene({ source: "file", initialData: (props as FileModeProps).initialData });
        if (cancelled || !scene) { setIsInitialized(true); return; }
        ...
        if (initialElements.length > 0 && scene.elements.length > 0) {
          const shouldOverride = await new Promise<boolean>((resolve) => {
            ...
            modal.querySelector(".cancel-btn")?.addEventListener("click", () => { resolve(false); document.body.removeChild(modal); });
            modal.querySelector(".replace-btn")?.addEventListener("click", () => { resolve(true); document.body.removeChild(modal); });
            modal.addEventListener("click", (e) => { if (e.target === modal) { resolve(false); document.body.removeChild(modal); } });
          });
          if (!shouldOverride) { setIsInitialized(true); return; }
        }
