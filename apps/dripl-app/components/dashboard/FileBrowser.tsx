    ...
    <div className="flex-1 p-8 overflow-auto bg-background">
      <div className="flex gap-4 mb-12">
        <button 
          ...
          className="flex flex-col items-center justify-center w-48 h-32 rounded-xl border border-border bg-card hover:bg-accent/20 transition-all group"
        >
          <Plus className="h-8 w-8 text-muted-foreground group-hover:text-foreground mb-2 transition-colors" />
          <span className="text-sm font-medium text-muted-foreground group-hover:text-foreground">Create a Blank File</span>
        </button>
          ...
          {files.map((file) => (
            ...
            >
              <div className="col-span-6 flex items-center gap-3">
                ...
                <span className="text-sm text-foreground font-medium group-hover:text-accent-foreground">{file.name}</span>
              </div>
